...
#define SPI_PORT SPI1 // Your desired SPI port.       Used only when "USE_SPI" is defined
...
void setup()
{
  ...
  success &= (myICM.setDMPODRrate(DMP_ODR_Reg_Gyro_Calibr, 0) == ICM_20948_Stat_Ok); // Set to 225Hz
  success &= (myICM.setDMPODRrate(DMP_ODR_Reg_Cpass_Calibr, 0) == ICM_20948_Stat_Ok); // Set to 225Hz
  ...
  if ((myICM.status == ICM_20948_Stat_Ok) || (myICM.status == ICM_20948_Stat_FIFOMoreDataAvail)) // Was valid data available?
  {
    ...
    if ((data.header & DMP_header_bitmap_Accel) > 0) // Check for Accel
    {
      ...
      SERIAL_PORT.println(acc_z);
       ...
       if ( (data.header & DMP_header_bitmap_Gyro_Calibr) > 0 ) // Check for Gyro
       {
         float x = (float)(data.Raw_Gyro.Data.BiasX); // Extract the raw gyro data
         float y = (float)(data.Raw_Gyro.Data.BiasY);
         float z = (float)(data.Raw_Gyro.Data.BiasZ);
       ...
       {
         int x = data.Compass_Calibr.Data.X; // Extract the compass data
         int y = data.Compass_Calibr.Data.Y;
         int z = data.Compass_Calibr.Data.Z;
